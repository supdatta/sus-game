...
import LevelEcoPointsBar from "../components/ui/LevelEcoPointsBar"; // Import the new component
...
import hangmanImg from "/Rescue_Mission.png";
import quizImg from "/quiz game.png";
import wordSearchImg from "/wordsearch.png";
import crosswordImg from "/crossword.png";
...
const PlayIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-white" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg> );
...
const DashboardPage: React.FC = () => {
    ...
    const { user } = authContext!; // Safely access user data

    const [levelProgress, setLevelProgress] = useState<Record<string, number>>({});
    const [continueQuest, setContinueQuest] = useState<any>(null);

    const mainLevels = [
        { id: "energy-conserver", title: "Level 1: Energy Conserver", description: "Learn about energy conservation...", image: energyImg, locked: false, external: true },
    ...
    const minigames = [
        { id: "hangman", title: "Eco Hangman", description: "Guess the green words!", image: hangmanImg },
        { id: "word-search", title: "Word Search", description: "Find the hidden eco terms.", image: wordSearchImg },
        { id: "crossword", title: "Crossword", description: "Solve the sustainability puzzle.", image: crosswordImg },
        { id: "quiz", title: "Eco Quiz", description: "Test your eco-knowledge!", image: quizImg }
    ];

    useEffect(() => {
        if (!user) return; // Guard clause if user is not logged in
        const loadedProgress: Record<string, number> = {};
        mainLevels.forEach(level => {
            const saved = localStorage.getItem(`progress_${user.username}_${level.id}`);
            loadedProgress[level.id] = saved ? parseInt(saved, 10) : 0;
        });
        setLevelProgress(loadedProgress);

        const lastPlayedId = localStorage.getItem(`lastPlayed_${user.username}`);
        const questToContinue = mainLevels.find(l => l.id === lastPlayedId && !l.locked) || mainLevels.find(l => !l.locked);
        setContinueQuest(questToContinue);
    }, [user]);

    const handleLevelClick = (levelId: string) => {
        if (user) {
            localStorage.setItem(`lastPlayed_${user.username}`, levelId);
        }
        // If navigating to an external game, set a pending flag to grant points on return
        if (levelId === 'energy-conserver') {
            localStorage.setItem('pendingExternalGame', '1');
        }
        if (levelId === 'waste-dungeon') {
            localStorage.setItem('pendingExternalGame', '1');
        }
    };
    ...
    const ConditionalLink: React.FC<{quest: any, children: React.ReactNode, className?: string}> = ({ quest, children, className }) => {
        if (quest.external) {
            // For energy-conserver, open Cutscene.html
            const cutsceneUrl = `${import.meta.env.BASE_URL}energy-conserver/Cutscene.html`;
            return <a href={cutsceneUrl} className={className} onClick={() => handleLevelClick(quest.id)}>{children}</a>;
        }
        if (quest.id === 'waste-dungeon') {
            // For waste-dungeon, open basement.html
            const basementUrl = `${import.meta.env.BASE_URL}dungeon-level/basement.html`;
            return <a href={basementUrl} className={className} onClick={() => handleLevelClick(quest.id)}>{children}</a>;
        }
        // Correctly uses the simplified path for internal game routes
        return <Link to={`/game/${quest.id}`} className={className} onClick={() => handleLevelClick(quest.id)}>{children}</Link>;
    };

    return (
        <div className="min-h-screen bg-background p-4" style={{backgroundColor: "#F0D8C2"}}>
            <div className="container mx-auto max-w-5xl">
                {user && <LevelEcoPointsBar />}
                    ...
                    <div className="mb-12">
                        ...
                        <ConditionalLink quest={continueQuest} className="block group">
                            <PixelCard className="relative overflow-hidden p-0">
                                ...
                                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-green-600 border-2 border-white text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    <PlayIcon />
                                </div>
                            </PixelCard>
                        </ConditionalLink>
                ...
                <div>
                    ...
                    <PixelCard className="p-4 mb-4">
                      <p className="font-pixel text-xs text-muted-foreground">
                        How points work: Playing any game or quest grants +5 eco points when you exit it. Each EcoScan mission grants +10 points when you analyze an item. On the Rewards page, completing a mission grants +5 points.
                      </p>
                    </PixelCard>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {minigames.map(game => (
                            <Link to={game.id === "word-search" ? "/wordsearch" : `/game/${game.id}`} key={game.id} className="block group h-full">
